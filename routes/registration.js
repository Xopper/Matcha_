    ...
    return new Promise((res, rej) => {
        pool.getConnection((err, connection) => {
            if (err)
                ...
                connection.execute('SELECT COUNT(*) as count FROM `users` WHERE `email` = ?', [email], (err, result)=>{
                    if (err)
                        rej(err)
                    else{
    ...
    return new Promise((res, rej)=>{
        pool.getConnection((err, connection) =>{
            if (err) 
                ...
                connection.execute('INSERT INTO `users`(`user_name`,`first_name`,`last_name`,`email`, `password`, `token`, `authentication_token`,`biography`, `profile_img`, `img_one`, `img_two`, `img_three`, `img_four`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [userData.username, userData.firstName, userData.lastName, userData.email, userData.password, userData.token, userData.authToken, '', '', '', '', '', ''], (err, result)=>{
                    if (err) 
                        rej(err)
                    else{
                        const queryResult = result
...
const checkRegistrationData = async (req, res, next) =>{
    ...
    if (isEmpty(checkerResult) && resultEmail === 0 && resultUserName === 0)
    {
        ...
        const validationToken = jwt.sign({userName: registerInputData.username}, "mafhamnachwalakinma3lichlhalwassaoulfanid04")
        registerInputData.token = validationToken
        //generate authentication jwt token
        const authenticationToken = jwt.sign({userName: registerInputData.username}, "boul3al7ayat7obilanamnghirakma3ichach7obi00")
        ...
        {
            if (resultEmail === 1)
                regErrors.email = "Email already exists"
            if (resultUserName === 1)
                regErrors.lastName = "User Name already exists"
            req.registrationErrors = regErrors;
            next()
        }else{
