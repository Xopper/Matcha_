...
function updateGenderInterest(userId, values)
{
    return new Promise((resolve, reject) =>{
        pool.getConnection((err, connection) =>{
            if (err) reject(err)
            connection.execute('UPDATE `users` SET `gender` = ?, `sexual_preference` = ? WHERE `id` = ?', [values.gender, values.interests, userId], (err, result) =>{
                if (err) reject(err)
                else{
                    const queryResult = result;
                    connection.release();
...
const primaryValidation = (req, res, next)=>{
    ...
    if (values.gender === '' || typeof values.gender === undefined) 
        primaryErrors.gender = 'Gender is required';
    ...
    if (values.tags.length === 0 || values.tags.length > 5)
        ...
        if (errlentag.length !== 0) 
            primaryErrors.tags = 'Tags length must be between 1 and 20 characters';
    }
